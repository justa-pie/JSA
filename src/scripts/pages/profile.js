...
async function initProfile(user) {
    ...
    try {
        ...
        // Merge auth info
        if (!profileData.avatarUrl && user.photoURL) profileData.avatarUrl = user.photoURL;
        if (!profileData.displayName && user.displayName) profileData.displayName = user.displayName;

    } catch { profileData = { displayName: user.displayName || "", email: user.email || "" }; }
...
function renderHero() {
    ...
    // Text fields
    document.getElementById("profileName").textContent  = profileData.displayName || currentUser.displayName || "Người dùng";
    ...
    if (date && !isNaN(date)) {
        joined.textContent = date.toLocaleDateString("vi-VN", { month: "long", year: "numeric" });
    }

    // Info chips
    const chips = [];
    if (profileData.dob)      chips.push(`<span class="info-chip"><i class="fa-solid fa-cake-candles"></i> ${formatDob(profileData.dob)}</span>`);
    if (profileData.phone)    chips.push(`<span class="info-chip"><i class="fa-solid fa-phone"></i> ${profileData.phone}</span>`);
    if (profileData.location) chips.push(`<span class="info-chip"><i class="fa-solid fa-location-dot"></i> ${profileData.location}</span>`);
...
window.openEditModal = function () {
    document.getElementById("editName").value     = profileData.displayName || currentUser?.displayName || "";
...
window.saveProfile = async function () {
    ...
    if (!name) { errEl.textContent = "Tên không được để trống."; errEl.classList.add("show"); return; }
    try {
        btn.disabled  = true;
    ...
    } catch { errEl.textContent = "Có lỗi xảy ra. Thử lại sau."; errEl.classList.add("show"); }
...
async function loadFavorites() {
    ...
    try {
        const snap  = await firebase.firestore().collection("users").doc(currentUser.uid)
            .collection("favorites").orderBy("savedAt","desc").limit(50).get();
        const items = snap.docs.map(d => d.data());
        document.getElementById("statFavorites").textContent = items.length;
        if (!items.length) { renderEmpty("favorites"); return; }
        document.getElementById("profileTabContent").innerHTML =
            `<div style="display:flex;flex-direction:column;gap:.5rem">
                ${items.map((s,i) => renderSongRow(s, i, "favorites")).join("")}
            </div>`;
    } catch { renderError("Không tải được danh sách yêu thích."); }
}
...
async function loadHistory() {
    ...
    try {
        const snap  = await firebase.firestore().collection("users").doc(currentUser.uid)
            .collection("history").orderBy("viewedAt","desc").limit(50).get();
        const items = snap.docs.map(d => d.data());
        document.getElementById("statHistory").textContent = items.length;
        if (!items.length) { renderEmpty("history"); return; }
        document.getElementById("profileTabContent").innerHTML =
            `<div style="display:flex;flex-direction:column;gap:.5rem">
                ${items.map((s,i) => renderSongRow(s, i, "history")).join("")}
            </div>`;
    } catch { renderError("Không tải được lịch sử xem."); }
}
...
async function loadPlaylists() {
    ...
    try {
        const snap  = await firebase.firestore().collection("users").doc(currentUser.uid)
            .collection("playlists").orderBy("createdAt","desc").limit(30).get();
    ...
    } catch { renderError("Không tải được danh sách playlist."); }
}
...
async function loadPhotos() {
    ...
    try {
        const snap  = await firebase.firestore().collection("users").doc(currentUser.uid)
            .collection("photos").orderBy("createdAt","desc").limit(30).get();
        const items = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    ...
    } catch { renderError("Không tải được ảnh."); }
}
...
window.handlePhotoUpload = async function (e) {
    ...
    try {
        ...
        await firebase.firestore().collection("users").doc(currentUser.uid)
            .collection("photos").add({ base64, createdAt: Date.now() });
...
window.deletePhoto = async function (photoId, btn) {
    ...
    try {
        ...
        loadPhotos();
    } catch { showToast("Không xoá được, thử lại.", "error"); }
};
...
window.openEditPlaylist = function (plId, name, coverUrl) {
    ...
    if (coverUrl) {
        prev.src = coverUrl; prev.style.display = "block";
...
window.savePlaylist = async function () {
    ...
    if (!name) { errEl.textContent = "Tên playlist không được để trống."; errEl.classList.add("show"); return; }
    try {
        btn.disabled  = true;
        btn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> Đang lưu...';
        const col  = firebase.firestore().collection("users").doc(currentUser.uid).collection("playlists");
        const data = { name, coverUrl: window._plCoverDataUrl||"", updatedAt: firebase.firestore.FieldValue.serverTimestamp() };
        if (mode === "create") {
            await col.add({ ...data, songCount:0, songs:[], createdAt: firebase.firestore.FieldValue.serverTimestamp() });
        ...
        closePlModal();
        loadPlaylists();
    } catch { errEl.textContent = "Có lỗi xảy ra."; errEl.classList.add("show"); }
    finally   { btn.disabled = false; btn.innerHTML = btn.dataset.label; }
};
...
window.deletePlaylist = async function (plId) {
    ...
    try {
        await firebase.firestore().collection("users").doc(currentUser.uid)
            .collection("playlists").doc(plId).delete();
        closePlModal();
        showToast("Đã xoá playlist", "success");
        loadPlaylists();
    } catch { showToast("Không xoá được.", "error"); }
};

// ─── Remove favorite / history ────────────────────────────────────────────────
window.removeFavorite = async function (songId, btn) {
    btn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin" style="font-size:13px"></i>';
    try {
        await firebase.firestore().collection("users").doc(currentUser.uid)
            .collection("favorites").doc(String(songId)).delete();
...
window.removeHistory = async function (songId, btn) {
    ...
    try {
        await firebase.firestore().collection("users").doc(currentUser.uid)
            .collection("history").doc(String(songId)).delete();
        removeRow(btn, "statHistory", "history");
    } catch { showToast("Không xoá được.", "error"); }
};
function removeRow(btn, statId, emptyType) {
    const row = btn.closest(".chart-item");
    Object.assign(row.style, { opacity:"0", transform:"translateX(20px)", transition:"all .25s ease" });
    setTimeout(() => {
        row.remove();
        const count = document.querySelectorAll("#profileTabContent .chart-item").length;
...
function renderEmpty(type) {
    const map = {
        favorites: { icon:"fa-heart",             text:"Chưa có bài hát yêu thích",  sub:"Nhấn ❤️ trên trang chi tiết bài hát để lưu" },
        history:   { icon:"fa-clock-rotate-left", text:"Chưa có lịch sử xem",        sub:"Lịch sử sẽ tự động ghi lại khi bạn xem bài hát" },
    };
