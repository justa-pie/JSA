            ...
            <div class="admin-sub-grid" style="grid-template-columns:1fr 1fr 1fr">
              <div>
                <div class="admin-sub-title">
                  ...
                  ${u._favsCount ? `<button class="btn btn-sm btn-danger" style="margin-left:auto;padding:2px 8px;font-size:.7rem"
              ...
              <div>
                <div class="admin-sub-title">
                  ...
                  ${u._histCount ? `<button class="btn btn-sm btn-danger" style="margin-left:auto;padding:2px 8px;font-size:.7rem"
...
const Admin = {
    ...
    applyFilters() {
        ...
        if (q)       list = list.filter((u) => (u.displayName || "").toLowerCase().includes(q) || (u.email || "").toLowerCase().includes(q) || u.id.toLowerCase().includes(q));
    ...
    clearFilters() {
        ["flt-name","flt-email","flt-date-from","flt-date-to","flt-min-favs"]
...
auth.onAuthStateChanged(async (user) => {
    ...
    try {
        ...
        if (avatarEl) {
            ...
            if (src) {
                ...
                avatarEl.textContent = (user.displayName || user.email || "A").substring(0, 1).toUpperCase();
            }
        }
    } catch {
        const avatarEl = document.getElementById("sidebar-avatar");
        if (avatarEl) avatarEl.textContent = (user.displayName || user.email || "A").substring(0, 1).toUpperCase();
    }
    ...
    try {
        ...
        const src2  = uDoc2.exists ? (uDoc2.data().avatarUrl || user.photoURL || "") : (user.photoURL || "");
        const navInit = document.getElementById("navUserInitial");
        if (navInit) {
            if (src2) navInit.innerHTML = `<img src="${src2}" style="width:100%;height:100%;object-fit:cover;border-radius:50%" referrerpolicy="no-referrer" />`;
            else navInit.textContent = (user.displayName || user.email || "A").substring(0, 1).toUpperCase();
        }
    } catch { /* ignore */ }
...
document.getElementById("confirm-ok").addEventListener("click", () => { UI.closeConfirm(); if (State.confirmCb) State.confirmCb(); });
document.getElementById("confirm-modal").addEventListener("click", (e) => { if (e.target === e.currentTarget) UI.closeConfirm(); });
...
document.addEventListener("click", (e) => {
    ...
    if (item) {
        ...
        drop.querySelector(".adrop-label").textContent = item.textContent.trim();
